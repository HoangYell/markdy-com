...
| **Semantic themes** | `paper`, `editorial`, `midnight`, `blueprint`, and `graphite` — consistent colors per node role and edge kind |
| **Focused diagram modes** | Opt-in `architecture`, `flowchart`, `tree`, `state`, and `sequence` layouts |
...
Selectors: `$nodes` targets every node, `$edges` targets structural and animated edges, and a group name targets its members. Themes: `paper` (light default), `editorial`, `midnight`, `blueprint`, and `graphite`.
