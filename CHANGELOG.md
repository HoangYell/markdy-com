...
### Fixed & Enhanced
- **🌓 Host Background Lightness Detection & Modern CSS Color Functions (`@markdy/renderer-dom`)**:
  - **Clean Host Element Boundary**: Updated host background luminance inspection to evaluate `container.parentElement` rather than the diagram container itself, preventing the diagram from misreading its own rendered canvas background during dynamic theme synchronization.
  - **Modern Color Support**: Enhanced `isDarkBgColor` to natively support `oklch(...)` and `oklab(...)` color formats used by Tailwind CSS 4 and modern frontend frameworks.
...
### Updated & Polished
- **Official Brand & Mascot Launch Assets**:
  - Refreshed high-resolution action mascot launch banners (`male-markdy-ads.webp` & `male-markdy-ads.png`).
  - Synchronized optimized WebP delivery assets across documentation and website public static directories (`website/public/images/male-markdy-ads.webp`).
- **Publishing & CI Workflow Resilience**:
  - Enhanced Open VSX Registry publishing tolerance in release and VS Code extension workflows for idempotent runs and conflict resilience.
...
### Changed
- **Gentle Top-Right Floating Badge (`@markdy/renderer-dom`)** — Moved the `Powered by Markdy` badge out of the bottom toolbar into the top-right corner of the diagram canvas as a gentle, semi-transparent floating pill. This prevents the playback control toolbar from wrapping onto a second line and preserves an elegant, minimal watermark.
