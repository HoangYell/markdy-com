...
For focused stories, the same syntax supports `type=flowchart`, `type=tree`, `type=state`, `type=sequence`, and `type=constellation`; `theme=editorial` is available for flat documentation scenes and `theme=nebula` for radial ones.
